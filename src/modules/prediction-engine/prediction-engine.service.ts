...
export class PredictionEngineService {
    ...
    async calculateVehicleHealth(vehicleId: number) {
        ...
        if (vehicle.mileageHistory.length > 1) {
            ...
            const lastRecord = vehicle.mileageHistory[vehicle.mileageHistory.length - 1];
        ...
        const rules = await this.prisma.serviceRule.findMany({ include: { icon: true } });
        ...
        const predictions = rules.map(rule => {
            ...
            const kmLastService = lastService ? lastService.mileage : vehicle.initialMileage;
            const kmSinceLastService = vehicle.currentMileage - kmLastService;

            // Salud = 100 - ((Km Actual - Km Ultimo Servicio) / Intervalo * 100)
            let healthPercentage = 100 - ((kmSinceLastService / rule.frequencyKm) * 100);
