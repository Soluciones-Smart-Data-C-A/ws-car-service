...
export class ServiceRuleController {
    ...
    @ApiResponse({ status: 201, description: 'Regla de servicio creada exitosamente.' })
    ...
    @ApiResponse({ status: 200, description: 'Regla de servicio actualizada exitosamente.' })
    @ApiResponse({ status: 404, description: 'Regla de servicio no encontrada.' })
    update(@Param('id', ParseIntPipe) id: number, @Body() updateServiceRuleDto: UpdateServiceRuleDto) {
        return this.serviceRuleService.update(id, updateServiceRuleDto);
    }

    @Delete(':id')
    @ApiOperation({ summary: 'Eliminar una regla de servicio' })
    @ApiResponse({ status: 200, description: 'Regla de servicio eliminada exitosamente.' })
    @ApiResponse({ status: 404, description: 'Regla de servicio no encontrada.' })
