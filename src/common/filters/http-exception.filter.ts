    ...
    if (exception instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      switch (prismaException.code) {
        case 'P2002': // Violación de restricción única (ej: email duplicado)
          ...
          const target = prismaException.meta?.target as string[];
          message = `El valor ingresado para el campo ${target ? target.join(', ') : ''} ya existe.`;
          break;
        case 'P2003': // Violación de restricción de llave foránea
          status = HttpStatus.BAD_REQUEST;
          message =
            'Un registro relacionado (vehículo o servicio) no existe en la base de datos.';
          break;
        ...
        default:
          ...
          this.logger.error(
            `Prisma Error: ${prismaException.code}`,
            prismaException.stack,
          );
          break;
      }
    }
